...
Called through file-coding-system-alist, before the file is visited for real."
	(and (eq (car arg-list) 'insert-file-contents)
		 ...
		 (if (re-search-forward
		      ...
		      nil t)
		     (let* ((charset (intern (downcase (po-match-string 1))))
			    (slot (assq charset
					po-content-type-charset-alist)))
		       (list (cond (slot (cdr slot))
				   ((memq charset (coding-system-list))
				    charset)
				   (t 'no-conversion))))
		   '(no-conversion))))))
...
(defun po-validate ()
  ...
  ;; Do it all, using our own compilation buffer and command.
  (let ((compilation-buffer-name-function
	 (function (lambda (mode-name) (concat "*" mode-name " validation*"))))
...
(defun po-send-mail ()
  ...
  (let* ((team-flag (y-or-n-p
		     (_"\
Write to your team?  (`n' if writing to the TP robot) ")))
	 (address (if team-flag
		      (po-guess-team-address)
		    po-translation-project-address)))
    (if (not (y-or-n-p (_"Include current PO file in invoice? ")))
